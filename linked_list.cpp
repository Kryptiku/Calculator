...
// apending
void addNode(struct Nodehead, int n) {
...
struct NodesearchNode(struct Node head, int n) {
    Nodecur = head;
    while(cur) {
...
void copyLinkedList(struct Nodenode, struct Node pNew)
{
    if(node != NULL) {
        pNew = new Node;
        (pNew)->data = node->data;
...
int compareLinkedList(struct Nodenode1, struct Node node2)
{
    static int flag;
